...
impl Database {
  ...
  pub async fn create_execution(
    ...
    self
      .set_project_id_in_tx(&mut tx, project_id, is_admin)
      .await?;
      ...
      if let Some(parent_id) = parent_execution_id {
        if let Some(step_key_value) = step_key {
          ...
          if let Some(parent_row) = parent_row {
            let parent_parent_execution_id: Option<Uuid> = parent_row.get("parent_execution_id");
            let parent_root_execution_id: Option<Uuid> = parent_row.get("root_execution_id");
    ...
    if wait_for_subworkflow {
        ...
        (parent_id_for_wait, waiting_step_key.as_ref())
      {
        ...
        if let Some(parent_row) = parent_row {
          ...
          let execution_ids: Vec<String> = results.iter().map(|(id, _)| id.to_string()).collect();
    ...
    if batch_id.is_none() {
      ...
      let mut outputs_dict: serde_json::Map<String, serde_json::Value> =
        if let Some(row) = existing_row {
          if let Ok(Some(outputs_json)) = row.try_get::<Option<serde_json::Value>, _>("outputs") {

...
impl Database {
  ...
  pub async fn claim_next_executions(&self, worker_id: &Uuid) -> anyhow::Result<Option<Execution>> {
  ...
  pub async fn can_claim_execution_by_queue_limit(
    ...
    match result {
      Some(row) => {
        ...
        Ok(concurrency_limit.is_none() || running_count < concurrency_limit.unwrap_or(0) as i64)
      }
      None => {
  ...
  pub async fn get_worker_recovery_info(
    ...
    let result: Option<(String, Option<i32>, Option<i32>, Option<DateTime<Utc>>)> = sqlx::query_as(

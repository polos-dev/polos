        ...
        loop {
            ...
            .await
          {
            Ok(pending_executions) => {
              ...
              for (execution_id, assigned_to_worker, push_endpoint_url, cancelled_at) in
                pending_executions
              {
                // Check if cancelled_at is more than 2 minutes ago
                let should_directly_cancel = if let Some(cancelled_at_time) = cancelled_at {
                  ...
                  (assigned_to_worker, push_endpoint_url)
                {
                  ...
                  .await
                  {
                    ...
                    api::workers::CancelRequestResult::Error(api::workers::PushError::Network(
                      _,
                    )) => {

...
async def demo_api_queue(client: PolosClient):
    ...
    # Invoke multiple in parallel (will be queued by concurrency limit)
...
async def demo_db_queue(client: PolosClient):
    ...
    # Invoke multiple operations (all share same queue)
    print_section("Mixed read/write operations (shared queue)")
    read_handle = await db_read_workflow.invoke(client, {"table": "orders"})
    write_handle = await db_write_workflow.invoke(
        client, {"table": "orders", "data": {"product": "Widget"}}
    )
...
async def demo_heavy_queue(client: PolosClient):
...
async def demo_inline_and_named_queues(client: PolosClient):
    ...
    result = await inline_queue_workflow.run(client, {})
    print(f"  Result: {result['message']}")

    # Named queue
    print_section("Named queue configuration")
    print("Workflow uses string queue name: queue='my-named-queue'")
    result = await named_queue_workflow.run(client, {})
    print(f"  Result: {result['message']}")


async def demo_batch_processor(client: PolosClient):
    ...
    result = await batch_processor.run(client, {"items": items})
    print(f"  Items processed: {result['processed']}")
    print(f"  All items share the api-calls queue (limit: 5)")


async def demo_queue_orchestrator(client: PolosClient):
    ...
    result = await queue_orchestrator.run(client, {})
...
async def demo_runtime_concurrency(client: PolosClient):
    """Demonstrate setting queue concurrency at runtime.

    Shows the difference between concurrency=1 (sequential) and concurrency=3 (parallel).
    """
    ...
    # Use client.invoke() directly to set queue_concurrency_limit at runtime
    handles = []
    for i in range(num_workflows):
        handle = await client.invoke(
            ...
            queue_concurrency_limit=1,  # Only 1 workflow at a time
        )
        handles.append(handle)
    ...
    print("(Watch the worker output until it completes)")
    ...
    for i in range(num_workflows):
        handle = await client.invoke(
            workflow_id="slow_workflow",
            payload={"workflow_id": i + 10, "sleep_seconds": sleep_seconds},  # Use different IDs
            queue_name="parallel-demo-queue",
            queue_concurrency_limit=3,  # All 3 workflows can run at once
        )
        handles.append(handle)
    ...
    # Wait for all to complete
    print("\nWaiting for all workflows to complete...")
    print("(Watch the worker output - all should start at once!)")
...
async def main():
    ...
    try:
        await demo_api_queue(client)
        await demo_db_queue(client)
        await demo_heavy_queue(client)
        await demo_inline_and_named_queues(client)
        await demo_batch_processor(client)
        await demo_queue_orchestrator(client)
        await demo_runtime_concurrency(client)

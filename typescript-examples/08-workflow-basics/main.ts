...
async function runWorkflowDemos() {
  ...
  try {
    const handle = await client.invoke(simpleWorkflow.id, { name: 'Alice' });
  ...
  try {
    const handle = await client.invoke(dataPipeline.id, {
  ...
  try {
    const handle = await client.invoke(randomWorkflow.id, {});
    const result = await handle.getResult();
    console.log(`Result: ${JSON.stringify(result)}`);
  } catch (e) {
    console.log(`Error: ${String(e)}`);
  ...
  try {
    const handle = await client.invoke(parentWorkflow.id, {
      items: [
  ...
  try {
    const handle = await client.invoke(orchestratorWorkflow.id, {

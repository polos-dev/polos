 ...
 *   POLOS_DEPLOYMENT_ID - Deployment ID (default: workflow-basics-examples)
 */
...
async function main() {
  ...
  const deploymentId = process.env['POLOS_DEPLOYMENT_ID'] ?? 'workflow-basics-examples';
  ...
  const workflowIds = [
    ...
    timedWorkflow.id,
    randomWorkflow.id,
    validateAndEnrich.id,
    parentWorkflow.id,
    orchestratorWorkflow.id,
  ];

  console.log('Starting Workflow Basics Examples worker...');
  console.log(`  Project ID: ${projectId}`);
  console.log(`  Workflows: [${workflowIds.join(', ')}]`);

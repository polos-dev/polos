/**
 * Polos Worker for the Suspend/Resume example.
 *
 * Run with:
 *   npx tsx worker.ts
 *
 ...
 *   POLOS_DEPLOYMENT_ID - Deployment ID (default: suspend-resume-examples)
 */
...
async function main() {
  ...
  const deploymentId = process.env['POLOS_DEPLOYMENT_ID'] ?? 'suspend-resume-examples';
  ...
  const worker = new Worker({
    ...
    projectId,
    deploymentId,
    workflows: [approvalWorkflow, multiStepForm, documentReview],
  });

  console.log('Starting Suspend/Resume Examples worker...');
  console.log(`  Project ID: ${projectId}`);
  console.log(`  Workflows: [${approvalWorkflow.id}, ${multiStepForm.id}, ${documentReview.id}]`);

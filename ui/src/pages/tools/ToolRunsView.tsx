...
export const ToolRunsView: React.FC = () => {
  ...
  const fetchRuns = async (customFilters?: {
    ...
    try {
      ...
      // Fetch all tool runs (no toolId filter)
      const runsData = await api.getWorkflowRuns(
        selectedProjectId,
        'tool',
        undefined, // No toolId filter - get all tool runs
        100,
        0,
        startTimeISO,
        endTimeISO
      ...
      setError(err instanceof Error ? err.message : 'Failed to load tool runs');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-500">Loading tool runs...</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="p-3 border-b border-gray-200 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">Tool Runs</h2>
        <Button
          variant="ghost"
          size="sm"
